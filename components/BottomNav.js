...
export default function BottomNav({ waNumber }) {
  /**
   * Each item consists of a Next.js Link (or anchor for external WA) and an
   * inline SVG icon. We avoid including text labels so the nav remains
   * uncluttered on small screens. Accessible aria-labels are added for
   * screen readers.
   */
  const items = [
    {
  ...
  return (
    <nav className="bottomNav">
      {items.map((item, idx) => (
        <Link key={idx} href={item.href} aria-label={item.label} legacyBehavior>
          <a className="navIcon">
            {item.icon}
          </a>
        </Link>
